...
void 
TextMan::Initialize(ShaderMan* shaderMan, const char* sFontTexturePath)
{
	Shader* pTextShader = new Shader();
	pTextShader->AddSource(Shader::ShaderType::Vertex, "textRenderingShader.vert");
	pTextShader->AddSource(Shader::ShaderType::Fragment, "textRenderingShader.frag");
	pTextShader->SetupShaders();

	shaderMan->AddShader("TextShader", pTextShader);

	//glUseProgram(m_pTextShader->ProgramID());
...
void 
TextMan::PrintText(ShaderMan* shaderMan, const char* sText, int x, int y, int size)
{
	...
	GLuint textShaderProgramID = shaderMan->GetShader("TextShader");
	glUseProgram(textShaderProgramID);

	// Bind texture
	m_pTextTexture->Bind(0, textShaderProgramID);
